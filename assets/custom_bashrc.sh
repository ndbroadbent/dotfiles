...
function _prj_rebuild_cache() {
  ...
  _prj_find_git_repos > $src_dir/.git_index
  # Append extra commands
  cat >>$src_dir/.git_index <<EOF
--rebuild-cache
--update-all
EOF
  echo -e "===== Cached $_bld_col$(_prj_repo_count)$_txt_col repos in $src_dir/.git_index"
}
...
function _prj_git_pull_or_status() {
...
function _prj_git_update_all() {
  echo -e "== Updating code in $_bld_col$(_prj_repo_count)$_txt_col repos...\n"
